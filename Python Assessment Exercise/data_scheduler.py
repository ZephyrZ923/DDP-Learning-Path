...
def date_passed(todays_date, scheduled_date):
    ...
    # Function to convert the date strings to datetime objects
    def convert_date(date_string):
        day, month = date_string.split()
        day = int(day[:-2])  # Remove 'th', 'st', 'nd', or 'rd' from day
        month = month_names.index(month) + 1  # Get the month's numeric value\

        # To create a valid datetime object, assuming the same current year
        # we use the current year (datetime.now().year) to extract just the year part from it
